...
void InProcessVideoCaptureDeviceLauncher::LaunchDeviceAsync(
  ...
  switch (stream_type) {
    ...
    // wangbo start external video
    case blink::mojom::MediaStreamType::EXTERNAL_VIDEO_CAPTURE: {
      start_capture_closure = base::BindOnce(
        &InProcessVideoCaptureDeviceLauncher::
              DoStartExternalCaptureOnDeviceThread,
          base::Unretained(this), device_id, params,
          CreateDeviceClient(media::VideoCaptureBufferType::kSharedMemory,
                            kMaxNumberOfBuffers, std::move(receiver),
                            std::move(receiver_on_io_thread)),
...
// wangbo start external video
void InProcessVideoCaptureDeviceLauncher::DoStartExternalCaptureOnDeviceThread(
    const std::string& device_id,
    const media::VideoCaptureParams& params,
  ...
  video_capture_device = ExternalVideoCaptureDevice::Create(device_id);
#endif
  if (!video_capture_device) {
    client->OnError(
      media::VideoCaptureError::kInProcessDeviceLauncherFailedToCreateDeviceInstance,
      FROM_HERE, "Could not create capture device");
    return;
  }
  if (video_capture_device)
    video_capture_device->AllocateAndStart(params, std::move(client));
  std::move(result_callback).Run(std::move(video_capture_device));
}
// wangbo end
