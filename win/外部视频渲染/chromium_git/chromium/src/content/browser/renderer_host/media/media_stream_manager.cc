...
void MediaStreamManager::SetUpRequest(const std::string& label) {
  ...
  // wangbo start external video
  const bool is_external_capture =
      request->video_type() == MediaStreamType::EXTERNAL_VIDEO_CAPTURE;
  if (is_external_capture && !SetUpExternalCaptureRequest(request)) {
    FinalizeRequestFailed(label,
                          request,
                          MediaStreamRequestResult::EXTERNAL_VIDEO_CAPTURE_FAILURE);
    return;
  }
  // wangbo end
...
// wangbo start external video
bool MediaStreamManager::SetUpExternalCaptureRequest(DeviceRequest* request) {
  DCHECK(request->video_type() == MediaStreamType::EXTERNAL_VIDEO_CAPTURE
    && request->audio_type() == MediaStreamType::NO_SERVICE);

  if (request->video_type() != MediaStreamType::EXTERNAL_VIDEO_CAPTURE ||
    request->audio_type() != MediaStreamType::NO_SERVICE) {
    LOG(ERROR) << "Invalid external capture request.";
    return false;
  }

  const std::string video_device_id = request->controls.video.device_id;
  const std::string audio_device_id = "";
