...
void VideoCaptureDeviceClient::OnIncomingCapturedYuvData(
  ...
  const auto reservation_result_code = ReserveOutputBuffer(
    frame_format.frame_size, PIXEL_FORMAT_I420, frame_feedback_id,
    &buffer);
  ...
  const int new_unrotated_width = frame_format.frame_size.width();
  const int new_unrotated_height = frame_format.frame_size.height();
  int destination_width = new_unrotated_width;
  int destination_height = new_unrotated_height;
  ...
  int yplane_stride, uv_plane_stride;
  GetI420BufferAccess(buffer, dimensions, &y_plane_data,
                      &u_plane_data,&v_plane_data, &yplane_stride,
                      &uv_plane_stride);
  ...
  if(clockwise_rotation > 360){
      libyuv::I420Mirror(y_data,y_stride,
                     u_data,u_stride,
                     v_data,v_stride,
                     y_plane_data, dst_y_stride,
                     u_plane_data, dst_u_stride,
    ...
    libyuv::I420Rotate(y_data,y_stride,
                     u_data,u_stride,
                     v_data,v_stride,
                     y_plane_data, dst_y_stride,
                     u_plane_data, dst_u_stride,
  ...
  VideoCaptureFormat output_format = frame_format;
  output_format.frame_size = dimensions;
