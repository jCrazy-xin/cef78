...
void InitializeVideoTrackControls(const blink::WebUserMediaRequest& web_request,
  ...
  *stream_type = MediaStreamType::NO_SERVICE;
  ...
  if (!source_constraint.IsEmpty()) {
    ...
    else if (source_constraint == blink::kMediaStreamSourceExternal) {
      *stream_type = MediaStreamType::EXTERNAL_VIDEO_CAPTURE;
    }
    // wangbo end
  } else {
...
void UserMediaProcessor::DelayedGetUserMediaRequestFailed(
  ...
  switch (result) {
    ...
    case MediaStreamRequestResult::SYSTEM_PERMISSION_DENIED:
      ...
      return;
    // wangbo start external video
    case MediaStreamRequestResult::EXTERNAL_VIDEO_CAPTURE_FAILURE:
      web_request.RequestFailed(
          blink::WebUserMediaRequest::Error::kExternalVideoCapture,
          "Failed External Video Capture");
      return;
    // wangbo end
  }
  NOTREACHED();
