...
// wangbo start crashpad unique hash
std::string ProcessSnapshotWin::ProcessType() {
  if (command_line_.HasSwitch("type"))
    return command_line_.GetSwitchValueASCII("type");
  else
    return "browser";
}
// wangbo end
...
void ProcessSnapshotWin::InitializePebData(
  ...
  if (process_parameters.CommandLine.Length) {
    command_line.resize(process_parameters.CommandLine.Length /
                        sizeof(wchar_t));
    if (process_reader_.Memory()->Read(
      ...
      command_line_ = base::CommandLine::FromString(command_line);
    }
  }
  // wangbo end
