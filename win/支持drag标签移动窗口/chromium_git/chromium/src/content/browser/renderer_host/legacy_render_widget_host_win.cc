...
// wangbo start move rootwindow
      move_rootwindow_enabled_(false) {
// wangbo end
  RECT rect = {0};
...
LRESULT LegacyRenderWidgetHostHWND::OnMouseRange(UINT message,
  ...
  if (move_rootwindow_enabled_){
    if (message == WM_MOUSEMOVE && (w_param & MK_LBUTTON)){
      ...
      int offY = pt.y - pt_buttondown_.y;
      if (offX != 0 || offY != 0){
        OffsetRect(&rt, offX, offY);
        ::MoveWindow(hRoot, rt.left, rt.top,
...
// wangbo start move rootwindow
void LegacyRenderWidgetHostHWND::SetMoveWindow(bool move_enabled) {
  move_rootwindow_enabled_ = move_enabled;
  SetCapture();
}
// wangbo end
