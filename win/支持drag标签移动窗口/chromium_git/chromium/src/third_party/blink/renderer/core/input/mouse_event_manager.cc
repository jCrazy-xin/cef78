...
bool MouseEventManager::TryStartDrag(
  ...
  if (move_rootwindow_enabled_) {
    drag_controller.StartMoveRootWindow();
    return false;
  }
  // wangbo end
...
WebInputEventResult MouseEventManager::DispatchDragEvent(
  ...
  // wangbo start move rootwindow
  WebInputEventResult result = event_handling_util::ToWebInputEventResult(
      drag_target->DispatchEvent(*me));
  move_rootwindow_enabled_ = me->MoveRootWindowEnabled();
  if(move_rootwindow_enabled_)
    me->preventDefault();
  return result;
  // wangbo end
